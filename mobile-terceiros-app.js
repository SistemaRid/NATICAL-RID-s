(function () {
  ...
  const STORAGE_KEYS = {
    ...
    leaders: "ridThirdPartyMobileOfflineLeaders",
    session: "ridThirdPartyMobileLastSession",
    ridFormSchema: "ridThirdPartyMobileRidFormSchema"
  };
  ...
  function pendingKey(uid) {
    return `ridThirdPartyMobilePending_${uid}`;
  }

  function cacheKey(uid) {
    return `ridThirdPartyMobileCache_${uid}`;
  }

  function syncKey(uid) {
    return `ridThirdPartyMobileLastSync_${uid}`;
  }

  function maintenanceKey(uid) {
    return `ridThirdPartyMobileMaintenances_${uid}`;
  }

  function maintenanceCacheKey(uid) {
    return `ridThirdPartyMobileMaintenancesCache_${uid}`;
  }

  function assignedRidCacheKey(uid) {
    return `ridThirdPartyMobileAssignedCache_${uid}`;
  }
  ...
  function getUserTypeLabel(userData) {
    ...
    const thirdPartyCompany = String(
      userData?.customFields?.terceiros?.value ||
      userData?.customFields?.terceiro?.value ||
      ""
    ).trim();
    ...
    const configuredType = String(
      userData?.type ||
      userData?.tipo ||
      userData?.customFields?.tipo?.value ||
      userData?.customFields?.type?.value ||
      userData?.customFields?.employmentType?.value ||
      userData?.customFields?.employmenttype?.value ||
      userData?.customFields?.employeetype?.value ||
      userData?.customFields?.emplooyetype?.value ||
      ""
    ).trim();

    if (configuredType) {
      if (configuredType.toUpperCase() === "TERCEIRO CONTRATADO" && thirdPartyCompany) {
        return `${configuredType} - ${thirdPartyCompany}`;
      }
      return configuredType;
    }

    const fallbackType = String(userData?.userType || "--").trim() || "--";
    if (fallbackType.toUpperCase() === "TERCEIRO CONTRATADO" && thirdPartyCompany) {
      return `${fallbackType} - ${thirdPartyCompany}`;
    }
    return fallbackType;
  }

  function hasThirdPartyMarker(value) {
    return String(value || "").trim().toUpperCase().includes("TERCEIR");
  }

  function isThirdPartyMobileUser(userData) {
    if (!userData || typeof userData !== "object") return false;
    if (String(userData?.customFields?.terceiros?.value || userData?.customFields?.terceiro?.value || "").trim()) {
      return true;
    }
    if (hasThirdPartyMarker(userData.employmentType)) return true;
    if (hasThirdPartyMarker(userData.userType)) return true;

    return Object.values(userData.customFields || {}).some((field) => {
      if (hasThirdPartyMarker(field?.value)) return true;
      if (hasThirdPartyMarker(field?.label)) return true;
      return false;
    });
  }

  function ensureThirdPartyMobileUser(userData) {
    if (!isThirdPartyMobileUser(userData)) {
      throw new Error("Este acesso mobile e exclusivo para terceiros cadastrados.");
    }
  }
  ...
  async function handleLogin(event) {
    ...
    try {
      ...
      if (state.online) {
        ...
        ensureThirdPartyMobileUser(offlineAuth.userData);
        ...
        <section class="mobile-shell">
          <header class="topbar panel">
            <div class="topbar-meta">
              ...
              <span class="muted" style="font-size:0.84rem;">${escapeHtml(getUserTypeLabel(state.currentUserData))}</span>
              <span class="muted" style="font-size:0.84rem;">${escapeHtml(state.currentUserData.sector || "Sem setor")}</span>
            </div>
            <div class="topbar-actions">
              <div class="topbar-action-row" style="flex-wrap:nowrap; align-items:center; justify-content:flex-end;">
  ...
  function bootstrapFromStoredSession() {
    ...
    if (!isThirdPartyMobileUser(session.userData)) return false;
  ...
  async function init() {
    ...
    try {
      ...
      if (!state.online && offlineAuth?.uid && offlineAuth?.userData && isThirdPartyMobileUser(offlineAuth.userData)) {
        state.currentUser = { uid: offlineAuth.uid };
        state.currentUserData = offlineAuth.userData;
        loadUserCache(offlineAuth.uid);
      ...
      if (!state.online && fallbackSession?.uid && fallbackSession?.userData && isThirdPartyMobileUser(fallbackSession.userData)) {
